...
function buatKode($tabel, $inisial){
    ...
    $qry = mysqli_query($koneksidb, "SELECT MAX(kode_booking) AS kode FROM $tabel");
    $data = mysqli_fetch_assoc($qry);

    // Jika belum ada data → mulai dari 0
    if($data['kode']) {
        $angka = (int) substr($data['kode'], strlen($inisial));
    } else {
        $angka = 0;
    }
    ...
    // Format dengan 6 digit angka setelah prefix
    return $inisial . sprintf("%06d", $angka);  // contoh: TRX000001
}
